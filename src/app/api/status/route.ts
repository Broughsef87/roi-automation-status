...
// Strip trailing slash so URL concatenation never produces a double //
const N8N_BASE = (process.env.N8N_BASE_URL || "http://localhost:5679").replace(/\/+$/, "");
...
export async function GET() {
  ...
  const wfUrl = `${N8N_BASE}/api/v1/workflows?limit=50`;
  ...
  try {
    ...
    if (!wfRes.ok) {
      // Surface the actual URL + a snippet of the response body so misconfig is obvious in the UI
      let body = "";
      try { body = (await wfRes.text()).slice(0, 200); } catch { /* ignore */ }
